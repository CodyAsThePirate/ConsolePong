...
int main(void){
	...
	do{
		...
		if(start=='s'||start=='S'){
			...
			if(modus=='6'){																			// 3 SÄTZE BIS 5
				...
				if(count1==4||count2==4){															// DIE LETZTEN COUNTER-WERTE NOCH MITBEKOMMT
					...
					if(count2==4)satz2++;															//
					for(int i=0; i<=4 ; i++){counter2[i]=' ';counter1[i]=' ';}						// BEI SIEG ZURÜCKSETZTEN ALLER WERTE
					if(satz1==2){system("cls");printf_s("\n\n\n\n\n\n\n       Spieler 1 hat gewonnen!!!");		//
						PlaySoundA("winner3.wav",NULL,SND_ASYNC);Sleep(5000);config=1;modus='0';satz1=0;satz2=0;}	//
					if(satz2==2){system("cls");printf_s("\n\n\n\n\n\n\n       Spieler 2 hat gewonnen!!!");		//
						PlaySoundA("winner3.wav",NULL,SND_ASYNC);Sleep(5000);config=1;modus='0';satz2=0;satz1=0;}	//
					count1=-1;count2=-1;															//
				}																					//
				
			}

			if(modus=='7'){																		// 1 MATCH BIS 11
				if(xBall>XVALUE-5&&count1<11&&config==0)count1++;								//	SPIELSTAND
				if(xBall<4&&count2<11&&config==0)count2++;										//
				field(control, modus, counter1, counter2, count1, count2);					// field() MUSS AUFGERUFEN WERDEN; DAMIT ES
				if(count1==11||count2==11){														// 
					system("cls");																// BEI SIEG ZURÜCKSETZTEN ALLER WERTE
					if(count1==11){printf_s("\n\n\n\n\n\n\n       Spieler 1 hat gewonnen!!!");	//
						PlaySoundA("winner3.wav",NULL,SND_ASYNC);};								//
					if(count2==11){printf_s("\n\n\n\n\n\n\n       Spieler 2 hat gewonnen!!!");	//
						PlaySoundA("winner3.wav",NULL,SND_ASYNC);};								//
					Sleep(5000);																//
		...
		fflush(stdin); xBall=0; control=1;
		system("cls");
...
int field(int control,char modi, char *count1, char *count2, int one,int two){
	...
	if(GetAsyncKeyState(0x4E) || control==1){     // DIES DARF NUR EINMAL VOR JEDEM SPIELSTART PASSIEREN!!
		...
		for(r=0; r<XVALUE-1; r++){for(i=0; i<YVALUE; i++){playingField[i][r]=' ';}}
	
		// ERSTELLEN DER MITTELLINIE DES playingFieldES
		for(i=0; i<YVALUE; i++){playingField[i][XMIDDLE]='|';}
        ...
        playingField[YMIDDLE][XMIDDLE]=1;	//ERSTELLT SPIELBALL

		// ERSTELLEN DES OBEREN SPIELRANDES
		printf_s("                 "); for(i=0; i<=XVALUE; i++){printf_s("%c", 254);} printf_s("\n");
		...
		printf_s("                 "); for(i=0; i<=XVALUE; i++){printf_s("%c", 254);}
	}
	...
	if(control!=1){  // DIES FINDET IMMER WÄHREND DER GAME FOR-SCHLEIFE IN DER MAIN-FUNKTION STATT
		for(i=0; i<=YVALUE-1 ;i++,fieldDelete.Y++){
			if(i==YMIDDLE){
				...
				SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), fieldDelete); // ALTERNATIVE ZU system("cls"); LÖSCHT NUR DAS playingField UND ÜBERSCHREIBT
				printf_s("%s", playingField[i]);}											// ES MIT DEM NEUEN; VORTEILE: FLIMMERN IST WEG! PROGRAMM IST SCHNELLER!
		}
		printf_s("\n");
	}
	...
	if(modi=='6'){
		if(satz==1.0){
			if((one==0||one==1||one==2||one==3||one==4) && os1-1<one)os1++;
			if((two==0||two==1||two==2||two==3||two==4) && ts1-1<two)ts1++;
			if(one==4){satz=1.5;}
			else if(two==4){satz=1.5;}
		}
		...
		if(satz==2.0){
			if((one==0||one==1||one==2||one==3||one==4) && os2-1<one)os2++;
			if((two==0||two==1||two==2||two==3||two==4) && ts2-1<two)ts2++;
			if(one==4){satz=2.5;}
			else if(two==4){satz=2.5;}
		}
		...
		if(satz==3.0){
			if((one==0||one==1||one==2||one==3||one==4) && os3-1<one)os3++;
			if((two==0||two==1||two==2||two==3||two==4) && ts3-1<two)ts3++;
			if(one==4){satz=0.5;}
			else if(two==4){satz=0.5;}
		}
...
int ballmove(int Ball,int *direction,int *upDown){
	...
	for(int i=0; i<YVALUE; i++){playingField[i][XMIDDLE]='|';}
	...
	if(*direction==-3){											// RÜCKWÄRTS-RUNTER
		*upDown+=1;Ball--;
		playingField[YMIDDLE+*upDown-1][Ball+1]=' ';			    // LÖSCHT ALTEN BALL MIT LEERZEICHEN
	...
	if(*direction==-1){											// RÜCKWÄRTS-GERADEAUS
		Ball--;
	...
	if(*direction==3){											// FORWÄRTS-RUNTER
		*upDown+=1;Ball++;
		playingField[YMIDDLE+*upDown-1][Ball-1]=' ';				// LÖSCHT ALTEN BALL MIT LEERZEICHEN
		playingField[YMIDDLE+*upDown][Ball]=1;						// SCHREIBT EINEN BALL EINE X/Y-STELLE VOR
		if(playingField[YMIDDLE+*upDown+1][Ball]==playingField[YVALUE][Ball]){	// FALLS SPIELRAND UNTEN
